...
class MatchManager
{
    ...
    public function addMatch(User $userConnected, User $match): int
    {
        ...
        if ($canAdd) {
            ...
            // si c'est mutuel alors on crée une discussion et l'ajout à chaque utilisateur
            if ($isMatchMutual) {
                $discussion = new Discussion();
                $discussion->setName($userConnected->getLastname() . ' | ' . $match->getLastname())
                    ->setToken(hash('sha256', random_bytes(32)));
                $discussion->addUser($userConnected);
                $discussion->addUser($match);
                ...
                // si l'utilisateur a l'option de recevoir des emails
            ...
            // si l'utilisateur connecté n'as pas d'abonnement alors on décompte ne nb de Spam
